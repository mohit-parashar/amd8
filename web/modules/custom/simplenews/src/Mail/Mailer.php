...
use Drupal\am_registration\EmailLoginController;
  ...
  public function sendCombinedConfirmation(SubscriberInterface $subscriber) {
    ...
    if ((count($changes) > 1 && $use_combined != 'never') || $use_combined == 'always') {
      ...
      foreach ($changes as $newsletter_id => $key) {
        ...
        $CreateLoginLink = new CreateLoginLinkController();
        $status_code = $bsdClient->emailRegister($subscriber->getMail());
        if($status_code == "error") {
          drupal_set_message("Some error occured","error");
        } else {
            $checkUser = user_load_by_mail($subscriber->getMail());
            // Check user is already exist or not in the drupal.
            if(empty($checkUser)) {
                  // Create user base on mail.
                  $language = \Drupal::languageManager()->getCurrentLanguage()->getId();
                  $user = \Drupal\user\Entity\User::create();
                  $name = $subscriber->getMail();
                  // Mandatory.
                 $pass = explode('@',$name);
                 ...
                 // Save user account.
                 $result = $user->save();
                 $CreateLoginLink->createLoginLink($user,'newsletter');
                   drupal_set_message("You will receive a confirmation e-mail shortly containing further instructions on how to complete your subscription.","status");
            } else {
                  $checkUser->activate();
                  // Save user account.
                  $checkUser->save();
                  $CreateLoginLink->createLoginLink($checkUser,'newsletter');
                  drupal_set_message("You will receive a confirmation e-mail shortly containing further instructions on how to complete your subscription.","status");
